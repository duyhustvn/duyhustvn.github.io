...
def print_step(step_num: int, desc: str):
    ...
    if PAUSE_ON_STEP:
        input(f"{YELLOW}  ⏸️  [Tạm dừng] Mở terminal khác để tự kiểm tra nếu muốn. Nhấn [Enter] để tiếp tục...{RESET}")
...
def lab1_handshake_and_listen_queue():
    ...
    print("Mục tiêu: Kiểm chứng trạng thái LISTEN và giải mã Recv-Q / Send-Q khi Client kết nối")
    ...
    print_step(1, f"Server gọi socket(), bind(127.0.0.1:{port}), listen(backlog={backlog})")
    run_ss_command(f"sport = :{port}", extra_flags="-tan", title="Trạng thái LISTEN ban đầu")

    # 2. Tạo 3 Client kết nối tới Server, NHƯNG Server CHƯA gọi accept()
    print_step(2, "3 Client đồng thời gọi connect() tới Server, nhưng Server CỐ TÌNH CHƯA gọi accept()")
    clients = []
    for i in range(3):
    ...
    time.sleep(0.1)
    run_ss_command(f"sport = :{port} or dport = :{port}", extra_flags="-tan", title="Sau khi 3 Client bắt tay xong")
...
""")
    ...
    time.sleep(0.1)
    run_ss_command(f"sport = :{port} and state established", extra_flags="-tan", title="Kết nối đã được accept")
...
def lab2_established_and_data_queues():
    print_banner("LAB 2: Trạng thái ESTABLISHED & Ý nghĩa Recv-Q / Send-Q khi truyền dữ liệu")
    print("Mục tiêu: Khám phá cách Recv-Q phản ánh dung lượng bộ đệm nhận (Receive Buffer) chưa đọc")
    ...
    run_ss_command(f"sport = :{port} or dport = :{port}", extra_flags="-tan", title="ESTABLISHED rỗng")

    print_step(2, "Server gửi 16,384 bytes (16KB) sang Client, nhưng Client CHƯA gọi recv()")
    payload = b"X" * 16384
    conn.sendall(payload)
    time.sleep(0.1)

    run_ss_command(f"sport = :{port} or dport = :{port}", extra_flags="-tan", title="Dữ liệu nằm chờ trong Receive Buffer")
...
""")
    ...
    time.sleep(0.1)
    run_ss_command(f"sport = :{port} or dport = :{port}", extra_flags="-tan", title="Sau khi Client đã đọc")
...
def lab3_fin_wait_2_and_close_wait_leak():
    print_banner("LAB 3: Chủ động đóng kết nối, TCP Half-Closed và Mô phỏng Rò rỉ CLOSE-WAIT")
    print("Mục tiêu: Kiểm chứng FIN-WAIT-2, CLOSE-WAIT và chứng minh tại sao CLOSE-WAIT không bao giờ tự mất")
    ...
    print_step(1, "Client là Bên chủ động đóng (Active Closer), gọi shutdown(SHUT_WR) để gửi gói FIN")
    ...
    run_ss_command(f"sport = :{port} or dport = :{port}", extra_flags="-tanp", title="FIN-WAIT-2 và CLOSE-WAIT")
...
""")
    ...
    print_step(3, "Kiểm chứng tính chất Half-Closed: Server vẫn có thể GỬI DỮ LIỆU sang Client!")
    conn.sendall(b"Server: Toi van con du lieu muon gui cho ban truoc khi dong!\n")
    received_in_fin_wait_2 = client.recv(1024)
    print(f"Client nhận được tin nhắn khi đang ở FIN-WAIT-2:\n  -> {YELLOW}{received_in_fin_wait_2.decode().strip()}{RESET}")

    print_step(4, "Mô phỏng BUG rò rỉ Socket (Socket Leak) trên Production:")
    print("Mở thêm 3 kết nối client khác và client đóng ngay, trong khi server 'quên' gọi close()...")
    leaked_clients = []
    leaked_conns = []
    for _ in range(3):
        c = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    ...
    time.sleep(0.2)
    run_ss_command(f"sport = :{port}", extra_flags="-tanp state close-wait", title="Tích tụ các socket kẹt CLOSE-WAIT")
...
""")
    ...
    print_step(5, "Giải cứu Server: Ứng dụng Server duyệt qua và gọi close() cho các socket bị bỏ quên")
    conn.close()
    ...
    srv.close()
    time.sleep(0.1)
    run_ss_command(f"sport = :{port}", extra_flags="-tan", title="Toàn bộ CLOSE-WAIT đã được giải phóng")
...
def lab4_time_wait_countdown_timer():
    ...
    print("Mục tiêu: Quan sát trạng thái TIME-WAIT và theo dõi bộ đếm thời gian thực bằng cờ `-o` trong ss")
    ...
    print_step(3, "Quan sát TIME-WAIT trên Client bằng cờ `ss -tan -o` (Hiển thị Timer):")
    run_ss_command(f"sport = :{port} or dport = :{port}", extra_flags="-tan -o", title="Thời điểm bắt đầu TIME-WAIT")
    ...
    run_ss_command(f"sport = :{port} or dport = :{port}", extra_flags="-tan -o", title="Timer sau 3 giây")
...
def lab5_server_as_active_closer():
    print_banner("LAB 5: Khi nào Server bị TIME-WAIT? (Xóa bỏ hiểu lầm 'Chỉ Client bị TIME-WAIT')")
    print("Mục tiêu: Chứng minh nếu Server chủ động ngắt kết nối trước (Keep-Alive Timeout), Server sẽ dính TIME-WAIT")
    ...
    print_step(1, "Server là bên CHỦ ĐỘNG ĐÓNG TRƯỚC (Ví dụ: Nginx ngắt kết nối do Client idle quá lâu)")
    conn.close()
    time.sleep(0.05)
    ...
    run_ss_command(f"sport = :{port} or dport = :{port}", extra_flags="-tan -o", title="Server bị TIME-WAIT trên chính port của mình")
...
def lab6_syn_sent_simulation():
    print_banner("LAB 6: Mô phỏng kẹt SYN-SENT (Khi gói tin bị DROP bởi Firewall hoặc mạng lỗi)")
    print("Mục tiêu: Tạo kết nối không phản hồi và quan sát trạng thái SYN-SENT cùng Send-Q")

    # Sử dụng dải RFC 5737 TEST-NET-1 (192.0.2.1) - dải IP được quy chuẩn không bao giờ phản hồi gói tin
    blackhole_ip = "192.0.2.1"
    blackhole_port = 80

    print_step(1, f"Client gọi connect() non-blocking tới địa chỉ hố đen {blackhole_ip}:{blackhole_port}")
    ...
    run_ss_command(f"dst {blackhole_ip}", extra_flags="-tan", title="Trạng thái SYN-SENT")
...
def interactive_menu():
    labs = [
        ("Lab 1: Khởi tạo LISTEN & Bí mật Recv-Q/Send-Q của Bắt tay 3 bước", lab1_handshake_and_listen_queue),
        ("Lab 2: ESTABLISHED & Cơ chế đệm dữ liệu (Recv-Q khi chưa đọc)", lab2_established_and_data_queues),
        ("Lab 3: Active Half-Close (FIN-WAIT-2) & Mô phỏng rò rỉ CLOSE-WAIT", lab3_fin_wait_2_and_close_wait_leak),
        ("Lab 4: Trạng thái TIME-WAIT & Bộ đếm thời gian 2MSL (-o timer)", lab4_time_wait_countdown_timer),
        ("Lab 5: Server làm Bên chủ động đóng (Ai sẽ chịu TIME-WAIT?)", lab5_server_as_active_closer),
        ("Lab 6: Mô phỏng kẹt SYN-SENT (Tắc nghẽn mạng / Firewall Drop)", lab6_syn_sent_simulation),
    ]

    while True:
        print("\n" + "=" * 65)
        print(f"{BOLD}{CYAN}=== HỆ THỐNG THỰC NGHIỆM KIỂM CHỨNG TRẠNG THÁI TCP & SS ==={RESET}")
        print("=" * 65)
        ...
        choice = input(f"{BOLD}Nhập lựa chọn của bạn (0-6 hoặc A): {RESET}").strip().upper()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Chương trình thực nghiệm kiểm chứng 11 trạng thái TCP và lệnh ss")
    parser.add_argument("--all", action="store_true", help="Tự động chạy toàn bộ các kịch bản thực nghiệm")
    parser.add_argument("--lab", type=int, choices=range(1, 7), help="Chạy một kịch bản Lab cụ thể (1-6)")
    parser.add_argument("-p", "--pause", action="store_true", help="Tạm dừng sau mỗi bước để mở terminal khác kiểm tra")
